...
test("illegal request fails with explicit request schema finding", () => {
  const badRequestPath = path.join(
    os.tmpdir(),
    `bad-request-${Date.now()}.json`,
  );
...
test("duplicate batch sanity emits warnings for overused blueprint/matchup/spell identity", () => {
  const duplicateRequestPath = path.join(
    os.tmpdir(),
    `duplicate-request-${Date.now()}.json`,
  );
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(duplicateRequest, null, 2),
  );
  ...
  assert.ok(
    reports.some((report) => report.balance_report.guardrail_status === "warn"),
  );

  const allWarningCodes = new Set(
    reports.flatMap((report) =>
      report.balance_report.validator_findings.map((finding) => finding.code),
    ),
  );
