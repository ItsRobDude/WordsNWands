...
export function validateRequestShape(request) {
  ...
  for (const field of requiredStringFields) {
    if (
      ...
      throw new Error(
        `Request validation failed: ${field} must be a non-empty string.`,
      );
    }
  }
  ...
  if (request.generator_version !== "structured_generation_v1") {
    throw new Error(
      `Request validation failed: generator_version must be structured_generation_v1.`,
    );
  }
  if (!REQUEST_MODES.has(request.generator_mode)) {
    throw new Error(
      `Request validation failed: unsupported generator_mode ${request.generator_mode}.`,
    );
  }
  if (!TARGET_TIERS.has(request.target_tier)) {
    throw new Error(
      `Request validation failed: unsupported target_tier ${request.target_tier}.`,
    );
  }
  if (!FAIL_RATE_BANDS.has(request.target_fail_rate_band)) {
    throw new Error(
      `Request validation failed: unsupported target_fail_rate_band ${request.target_fail_rate_band}.`,
    );
  }
  if (!OUTPUT_SCOPES.has(request.output_scope)) {
    throw new Error(
      `Request validation failed: unsupported output_scope ${request.output_scope}.`,
    );
  }
  if (!/^[0-9a-f]{32}$/.test(request.generation_seed)) {
    throw new Error(
      "Request validation failed: generation_seed must be 32 lowercase hex chars.",
    );
  }

  if (
    !Number.isInteger(request.encounter_count) ||
    request.encounter_count < 1 ||
    request.encounter_count > 12
  ) {
    throw new Error(
      "Request validation failed: encounter_count must be an integer in [1, 12].",
    );
  }
  ...
  if (typeof request.allow_name_generation_from_bank !== "boolean") {
    throw new Error(
      "Request validation failed: allow_name_generation_from_bank must be boolean.",
    );
  }
  if (typeof request.allow_flavor_generation_from_bank !== "boolean") {
    throw new Error(
      "Request validation failed: allow_flavor_generation_from_bank must be boolean.",
    );
  }
...
function validateStringArray(value, field) {
  if (
    ...
    throw new Error(
      `Request validation failed: ${field} must be a non-empty string array.`,
    );
  }
}
