...
for doc in "$CANONICAL_DOC" "$GAME_RULES_DOC" "$BALANCE_DOC"; do
  if ! rg -Fq "$EXPECTED_FINGERPRINT" "$doc"; then
...
if ! rg -Fq "damage_model_version = 'damage_model_v1'" "$CANONICAL_DOC"; then
  echo "[damage-model-consistency] Canonical doc missing version lock: ${CANONICAL_DOC#$ROOT_DIR/}" >&2
  exit 1
fi

if ! rg -Fq "$EXPECTED_VERSION" "$GAME_RULES_DOC" || ! rg -Fq "$EXPECTED_VERSION" "$BALANCE_DOC"; then
  echo "[damage-model-consistency] Non-canonical docs must reference ${EXPECTED_VERSION}" >&2
  exit 1
fi

if ! rg -Fq "Section 5.2.1, “Damage Model v1 (canonical)”" "$GAME_RULES_DOC"; then
  echo "[damage-model-consistency] Game rules doc must reference canonical section 5.2.1" >&2
  exit 1
fi

if ! rg -Fq "Section 5.2.1, “Damage Model v1 (canonical)”" "$BALANCE_DOC"; then
